...
class _SupplierSalesPerformanceReportPageState
  ...
  final List<Widget> rangePeriodEntries = [
    Padding(padding: const EdgeInsets.all(5.0), child: Text('Hari Ini')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('Minggu')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('Bulan')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('3 Bulan')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('6 Bulan')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('Tahun ini')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('5 Tahun')),
    Padding(padding: const EdgeInsets.all(5.0), child: Text('Semua')),
  ];
  ...
  Map brandChartFilter = {'rangePeriod': 5, 'valueType': 'sales_total'};
      ...
      filterForm: [
        SizedBox(
          ...
          child: AsyncDropdownMultiple<IposSupplier>(
            ...
            textOnSearch: (supplier) => "${supplier.code} - ${supplier.name}",
        ...
        SizedBox(
          ...
          child: Stack(
            children: [
              Text('Rentang Periode', style: _filterLabelStyle),
              Positioned(
                top: 20,
                child: ToggleButtons(
                  onPressed: (int index) {
  ...
  Future fetchCompareData() async {
    var supplierCodes = (supplierChartFilter['suppliers'] as List<IposSupplier>)
        .map<String>((e) => e.code)
        .toList();
    if (_supplier != null && !supplierCodes.contains(_supplier!.code)) {
      supplierCodes.add(_supplier!.code);
    }
    DateTimeRange range = convertToTimeRange(
